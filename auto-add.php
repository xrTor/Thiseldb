...
ob_start();
// The imdb.php file is included for its functions, but its output is suppressed.
...
/* ========= פונקציות עזר כלליות ========= */
function to_csv_val($arr){ if(!$arr) return null; $vals=[]; foreach((array)$arr as $x){ $t=trim((string)$x); if($t!=='') $vals[]=$t; } $vals=array_values(array_unique($vals)); return $vals?implode(', ',$vals):null; }
...
function replace_akas(mysqli $db, int $poster_id, string $imdb_id, array $akas): void { /* ... Your existing replace_akas function ... */ }
function sync_connections(mysqli $db, int $poster_id, string $source_imdb_id, array $connections_map): void { /* ... Your existing sync_connections function ... */ }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  // <<< תיקון: שינוי ברירת המחדל ל-skip
  $dup_mode = $_POST['dup_mode'] ?? 'skip';
  
  $ids = parse_ids($raw_input);
  if(!$ids){ $results[]=['tt'=>'N/A', 'ok'=>false, 'error'=>'לא נמצאו מזהי IMDb תקינים בקלט שהוזן.']; }
  else{
    foreach($ids as $tt){
      ...
      try{
        ...
        $rawRow = build_row($tt, $TMDB_KEY, $RAPIDAPI_KEY, $OMDB_KEY);
        ...
        if ($poster_id > 0 && strpos($res['action'], 'skipped') === false) {
            replace_akas($conn, $poster_id, $tt, $U['akas'] ?? []);
            $connections = imdb_connections_all($tt);
            sync_connections($conn, $poster_id, $tt, $connections);
        }
      } catch(Throwable $e){
      ...
      <?php foreach($results as $res): ?>
        <?php 
            ...
            if (!$res['ok']) {
                ...
                $status_text = 'כישלון';
            } elseif (strpos($status_text, 'skipped') !== false) {
                $status_class = 'res-skip';
                $status_text = 'דילוג';
            }
        ?>
        <div class="res <?= $status_class ?>">
          <strong><?= htmlspecialchars($res['tt']) ?>:</strong> 
          <?= htmlspecialchars(ucfirst($status_text)) ?>
          <?php if($res['error']): ?>
            <div style="font-size:13px; color: var(--err); margin-top:4px;">שגיאה: <?= htmlspecialchars($res['error']) ?></div>
          <?php endif; ?>
        </div>
      <?php endforeach; ?>
    </div>

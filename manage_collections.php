...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['create_collection'])) {
  ...
  if ($name !== '') {
    $stmt = $conn->prepare("INSERT INTO collections (name, description, image_url, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_collection'])) {
  ...
  $did = intval($_POST['delete_collection']);
  $conn->query("DELETE FROM collections WHERE id = $did");
  $conn->query("DELETE FROM poster_collections WHERE collection_id = $did");
  $message = "🗑️ האוסף נמחק";
}
...
<head>
  ...
  <style>
    ...
    .message { background:#ffe; padding:10px; border-radius:6px; border:1px solid #ddc; color:#333; margin-bottom:10px; text-align:center; }
  ...
  <?php while ($row = $result->fetch_assoc()): ?>
    <div class="collection-box">
      ...
      <form method="post" style="display:inline;">
        <?= csrf_input() ?>
        <button type="submit" name="delete_collection" value="<?= $row['id'] ?>" onclick="return confirm('למחוק את האוסף?')" class="link-btn">🗑️ מחק</button>
      </form>
